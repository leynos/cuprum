...
class TestLineEvent:
    """The per-line event value object."""

    def test_line_event_is_frozen(self) -> None:
...
class TestComposeLineCallbacks:
    """Composition of the per-line callback from the observe and user hooks."""

    def test_compose_returns_none_without_observers(self) -> None:
    ...
    def test_compose_stamps_elapsed_monotonic_time(
    ...
    def test_compose_fans_out_to_observe_output_event(self) -> None:
...
class TestFanOutHooks:
    """Fan-out over one or more line hooks."""

    def test_fan_out_hooks_runs_synchronous_hooks_in_order(self) -> None:
        ...
        assert outcome is None, "synchronous hooks must not return an awaitable outcome"
        assert received == ["first", "second"], (
            f"synchronous hooks must preserve registration order, got {received!r}"
        )

    def test_fan_out_hooks_awaits_async_outcomes_in_order(self) -> None:
    ...
    def test_fan_out_hooks_invokes_every_hook_before_awaiting(self) -> None:
    ...
    def test_fan_out_hooks_closes_skipped_coroutine_after_failure(self) -> None:
        ...
        assert inspect.iscoroutine(later_outcome), (
            "the later hook must return a coroutine"
        )
