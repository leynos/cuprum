...
def _events(draw: st.DrawFn) -> ExecEvent:
    ...
    return ExecEvent(
        ...
        stage_index=draw(st.none() | st.integers(min_value=0, max_value=5)),
        stage_count=draw(st.none() | st.integers(min_value=1, max_value=6)),
    )
...
class TestAdapterProjection:
    ...
    def _representative_event(phase: str) -> ExecEvent:
        ...
        # The fail-fast decision reports a failing stage, so it is the one
        # phase that carries a non-zero exit code, a duration, and a position
        # in the pipeline without being an ``exit`` event.
