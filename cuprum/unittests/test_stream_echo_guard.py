...
owns the narrow echo-guard behaviour added for issues #348 and #356: a
text-only sink whose encoding cannot represent subprocess output disables
echo for its drain while capture completes, exactly one structured warning is
logged with only closed-set categorical extras, and the final decoder flush
never re-attempts a rejected sink. Focused relay-diagnostics cases live in
``test_stream_relay_fallback_diagnostics.py``.
"""
