...
owns the narrow behaviour added for issues #348 and #356: a text-only sink
whose encoding cannot represent the subprocess output disables echo for its
drain while capture completes, exactly one structured warning is logged with
only closed-set categorical extras, the handled transition records one
``RelayFallback`` per affected drain, other I/O errors still propagate, and
the final decoder flush never re-attempts a rejected sink. Sinks exposing a
binary ``.buffer`` take the raw-bytes fast path and record nothing.
"""
