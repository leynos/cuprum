"""Environment-preservation tests for concurrent ``_EnvBackendSelector`` usage.

These tests verify that the backend environment variable (``CUPRUM_STREAM_BACKEND``)
is correctly isolated and restored when concurrent worker threads each hold the
backend selector lock, and that lock serialisation prevents one thread from
observing another thread's transient environment state.

Instrumented selector stand-ins (``_CoordinatedBackendSelector`` and
``_CheckpointBackendSelector``) inject coordination events and environment
observations at the selector boundary so that race conditions can be
...
from conftest import _EVENT_WAIT_TIMEOUT_SECONDS, join_and_assert_finished
