...
def test_non_int_concurrency_raises_type_error(
    ...
    with pytest.raises(TypeError, match=f"concurrency must be an int, got {type_name}"):
        ConcurrentConfig(concurrency=concurrency)
...
def test_non_ascending_failure_indices_are_rejected(failures: tuple[int, ...]) -> None:
...
@pytest.mark.parametrize(
    ...
    [
        ...
        pytest.param((True,), TypeError, "submission_indices must be ints", id="bool"),
