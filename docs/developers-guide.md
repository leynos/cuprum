...
| `refresh-sha`             | `get-codescene-sha.yml`  | `ubuntu-latest`       | none            | 10      |
...
Cranelift lint family. Every one of them is on the Ubicloud lane now; the fork
arm reads a `github-hosted` scope that no job writes, so a fork's pull request
compiles cold.
