...
- [x] (2026-09-24) Committed the gate-verified provenance fix as `d34feb4a`
  and reviewed that exact commit with CodeRabbit. The review found one minor
  plan mismatch: the synthesis step did not describe identity-based refresh of
  generated sinks by `dataclasses.replace`. Updated the step to match the
  implemented per-options provenance rule; `make markdownlint`,
  `make spelling`, and `make nixie` passed on the corrected plan. A final
  review of this correction remains.
...
5. After validation, synthesize in `_synthesize_sink_from_flags` while tracking
   which sink this options object generated. When `dataclasses.replace`
   forwards that generated sink, refresh it if the flags changed or clear it
   when both flags are false. Preserve caller-provided sinks unchanged, even if
   they are `GitHubActionsSink` instances or were generated by another options
   object. Record the synthesized sink with a private, non-comparing field so
   the check is by object identity rather than adapter type.
