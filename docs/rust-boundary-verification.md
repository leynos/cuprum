...
reports. Hosted Windows/macOS runtime execution passed on 2026-09-15: the
`Rust boundary verification` run 35004943625 succeeded with its
`Native contracts (windows-2022)`, `Native contracts (macos-latest)`, and
`Native contracts (ubuntu-latest)` jobs, and the `CI` run 35004943782 succeeded
with its `Extension-gated tests (Windows Python/Rust boundary)` and
...
The current boundary contract discovers all automatic and explicitly configured
Cargo target roots, requiring a source-level unsafe prohibition on each. Its
current measured run found the library and compile-contract target, compiled
six unsafe probes, and separately rejected unannotated future binary,
build-script, example, integration-test, and benchmark targets.
