...
| ----------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `rust/cuprum-streams`   | Safe stream orchestration, read/write and Linux `splice` policy, pump state machine, errors, and checked UTF-8 replacement.                                                               | `#![forbid(unsafe_code)]` at the crate root and in compile-contract tests. Unix receives `AsStream`/`OwnedStream`; Windows receives only synchronous-stream capabilities from the native boundary.                                                                                                                                                                         |
| `rust/cuprum-native-io` | Lifetime-bound OS borrows, unique writer adoption, single read/write/splice calls, checked native progress, Unix and Windows pipe fixtures, and close observation for native regressions. | Approved unsafe-bearing crate. Windows `SynchronousBorrowedStream` distinguishes a live borrow from a handle established as non-overlapped and synchronous. `adopt_writer` and `borrow_reader` are unsafe integration constructors with caller obligations. Every unsafe block has a local safety comment, and the crate denies unsafe operations inside unsafe functions. |
| `rust/cuprum-rust`      | Original `cdylib`/PyO3 module, argument validation, GIL release, error conversion, and the raw Python hand-off.                                                                           | Approved integration boundary. Its Unix path calls `adopt_writer` and `borrow_reader`; its Windows PyO3 entry points reject raw handles because they cannot establish the capability.                                                                                                                                                                                      |
...
Windows borrowed I/O carries a second, operational precondition beyond owner
lifetime. `rust/cuprum-native-io/src/windows.rs:16-45` makes
`SynchronousBorrowedStream` the capability consumed by `read_once`,
`write_once`, and `with_file` at
`rust/cuprum-native-io/src/windows.rs:141-165`. It represents a handle suitable
for blocking `ReadFile`/`WriteFile` semantics, not merely a live
`BorrowedHandle`. `synchronous_pipe` establishes that property for `CreatePipe`
endpoints at `rust/cuprum-native-io/src/windows.rs:91-105`; foreign Rust
integrations enter only through the documented unsafe constructors at
`rust/cuprum-native-io/src/lib.rs:76-129`. A bare Win32 handle has no
documented runtime mode query, so the property is established at creation,
duplication, or the audited hand-off rather than inferred from liveness.

The Python dispatch continues to decline native pumping on Windows because
`ProactorEventLoop` subprocess pipes are overlapped. The cross-reference at
`cuprum/_pipeline_streams.py:65-78` keeps that policy tied to the Rust
capability rather than treating the fallback as a lifetime-only decision.
PyO3's raw-handle entry points also reject Windows calls at
`rust/cuprum-rust/src/stream_pyfunctions.rs:80-85,128-133`, before they can
construct a synchronous capability.
...
| ---------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `cuprum-native-io::adopt_writer` (`src/lib.rs:62,85`)                                                      | Unix `OwnedFd::from_raw_fd` (`:66`) and Windows `OwnedHandle::from_raw_handle` (`:88`) require a valid, open, uniquely transferred resource that is never concurrently closed or reused. Drop closes once on normal, error, and real-unwind paths.                                                               | Unix PyO3 call at `rust/cuprum-rust/src/stream_pyfunctions.rs:93`; native tests `ownership_tests::{transferred_writer_closes_and_delivers_eof,owned_descriptor_reads_and_closes}` exercise adoption. Kani checks typed drop counts; native tests check close and EOF. |
| `cuprum-native-io::borrow_reader` (`src/lib.rs:100,124`)                                                   | Unix `BorrowedStream::borrow_raw` (`:104`) and Windows `BorrowedStream::borrow_raw` plus `SynchronousBorrowedStream::new_unchecked` (`:127,129`) require the owner to keep the same resource live for the returned lifetime and prevent close/reuse during GIL release, cancellation, and the callback.          | Unix PyO3 calls at `rust/cuprum-rust/src/stream_pyfunctions.rs:97,141`; native regression `ownership_tests::borrowed_reader_survives` exercises the raw borrow contract. Safe `borrow` uses an existing `AsStream` lifetime.                                          |
...
| Unix fixtures and observation (`src/lib.rs:160,184`)                                                       | `libc::pipe` (`:163`) fills two endpoint slots before `from_raw_fd` (`:169-173`) adopts them. `fcntl(F_GETFD)` (`:188`) observes close state and retries `EINTR` without granting ownership.                                                                                                                     | Native fixtures only. The fallible `descriptor_guard` `LockResult<MutexGuard>` serializes close observations in `ownership_tests.rs:16-20`.                                                                                                                           |
| Windows fixtures (`src/windows.rs:69-135`)                                                                 | `CreatePipe` (`:73-89`) fills handle slots before `from_raw_handle` adopts them. `synchronous_pipe` (`:91-105`) records its synchronous creation contract, and `GetHandleInformation` (`:118-135`) observes state without ownership.                                                                             | Windows tests use `synchronous_pipe` for the accepted path; cross-target checks compile the Windows native and streams crates.                                                                                                                                        |
| Windows borrowed I/O (`src/windows.rs:16-165`)                                                             | `SynchronousBorrowedStream` (`:16-45`) records blocking, non-overlapped suitability; `cap_std::fs::File::from_raw_handle` (`:153-165`) temporarily reconstructs a file, while `with_retained_owner` prevents closing the borrowed handle on return or unwind.                                                    | Private adapter used only by `read_once`/`write_once` (`:141-165`). Generic `BorrowedHandle` cannot reach this safe API; `new_unchecked` is the documented audited boundary. Locked dependency is `cap-std 4.0.3`; `rust/dylint.toml` has no filesystem exclusion.    |
| Windows rejected writer (`stream_pyfunctions.rs:54-59`)                                                    | `OwnedStream::from_raw_handle` (`:58`) requires a valid, exclusively transferred handle. Its immediate drop closes once, including if constructing the `OSError` unwinds, without asserting synchronous-I/O suitability.                                                                                         | Windows `rust_pump_stream` validates its arguments, consumes the promised writer transfer, and returns the raw-handle rejection. `test_rust_errno_windows::test_pump_rejects_raw_windows_handles` covers this safe Python boundary.                                   |
| PyO3 generated ABI (`rust/cuprum-rust/src/stream_pyfunctions.rs:72-73,121-122`; module `src/lib.rs:78-82`) | `#[pyfunction]`, `#[pyo3(signature = ...)]`, `#[pymodule]`, and `wrap_pyfunction!` generate/register wrappers. Unix `Python::detach` (`stream_pyfunctions.rs:42`) requires `Send`, no Python-state access, and valid resource lifetimes through completion. Windows raw-handle calls reject at `:80-85,128-133`. | Unix raw calls are `adopt_writer` (`stream_pyfunctions.rs:93`) and `borrow_reader` (`:97,140`). PyO3/CPython/asyncio/GIL behaviour remains trusted.                                                                                                                   |
...
Historical Round 35 integrated local checks are green: the Python suite
recorded 1,568 passes and one skip in `/tmp/issue379-round35-test.log`, the
Rust suite recorded 115/115 tests with no skips, and the extension-required
suite recorded 80 passes and one skip. That historical run included nine
unsafe-code compiler probes, Windows cross-Clippy, and the development build.
The cached Kani setup check had already passed in Round 33, which remains
retained in the preceding reports. Hosted Windows/macOS runtime execution
passed on 2026-09-15: the `Rust boundary verification` run 35004943625
succeeded with its `Native contracts (windows-2022)`,
`Native contracts (macos-latest)`, and `Native contracts (ubuntu-latest)` jobs,
and the `CI` run 35004943782 succeeded with its
...
`extended` Kani/Miri job is skipped on pull requests by design, so these hosted
runs add no Kani/Miri coverage. The second CodeRabbit review remains pending;
the first review passed with 0 findings.

The current boundary contract is distinct from Round 35's historical nine-probe
run. It discovers all automatic and explicitly configured Cargo target roots,
requiring a source-level unsafe prohibition on each. Its current measured run
found the library and compile-contract target, compiled six unsafe probes, and
separately rejected unannotated future binary, build-script, example,
integration-test, and benchmark targets.
[ADR-011's 2026-09-20 addendum](adr-011-audited-rust-boundaries.md#addendum-2026-09-20-lint-baseline-and-target-discovery)
records the implementation evolution, including Cargo's automatic-target
switches and the I/O boundary around discovery.
...
| ---------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `cuprum-native-io` borrowed and owned Unix/Windows handles | Borrowed readers remain caller-owned; transferred writers close once on normal, error, and real unwind; no released/reused descriptor is touched. Windows synchronous I/O additionally requires `SynchronousBorrowedStream`, so a live generic handle cannot enter `read_once`/`write_once`. | Kani exercises the production `with_owned_writer` drop counter where Verus cannot represent OS handles or destructors. The direct Verus resource assessment is recorded below.                                         | Valid-live-resource, unique-transfer, no-concurrent-close, and Windows non-overlapped I/O remain caller contracts. Kani's symbolic exit is bounded and has coverage witnesses. The writer callback receives an immutable reference, so it cannot replace or move the owned resource out of the scope. | Native Windows tests cover the accepted synchronous-pipe path and real unwind. The compile-fail contract rejects a generic borrowed handle. Hosted runtime execution remains separate evidence. | No runtime overlapped-handle fixture is retained because it is cumbersome and unstable in CI; the compile-time capability boundary remains the regression guard.                                                        |
| `cuprum-native-io` read/write kernels                      | Buffers expose only initialized in-bounds bytes; short reads/writes, `EINTR`, zero progress, and checked byte accounting preserve valid state.                                                                                                                                               | Verus verifies the exact production `progress.rs` bodies rendered by `scripts/render_boundary_proofs.py`; Kani checks the production progress kernels and ownership seam.                                              | `checked_progress` rejects overflow and oversized lengths; `record_write_progress` commits tail/count only after both checks. Aggregate pump/consume tracing counts retain saturating-add semantics, so they are not exact beyond `u64::MAX`.                                                         | Round 35 Rust checks covered 115/115 tests with no skips. Hosted Windows/macOS runtime execution was green in CI.                                                                               | Verus passed 2 functions with 0 errors; Kani/Miri and native tests remain scoped as recorded above. Neither verifier proves syscall effects.                                                                            |
| Linux `splice` adapter and fallback policy                 | Unsupported descriptors fall back; partial/interrupted transfers and broken-pipe drain preserve bytes and termination.                                                                                                                                                                       | Kani/Verus cover the typed decision, progress, and accumulation seams; the kernel call itself is external. Miri covers applicable native crate paths but excludes unshimmed `libc::splice`.                            | Syscall return/error guarantees, symbolic chunk and trace bounds, and non-vacuity witnesses are explicit assumptions or bounds.                                                                                                                                                                       | Round 35 local Rust and native checks passed; hosted Windows/macOS runtime execution was green in CI.                                                                                           | The syscall is not directly verified or Miri-interpreted because `libc::splice` has no supported shim. Round 27 Miri passed 13 tests with 0 ignored.                                                                    |
| `cuprum-streams` safe policy and decoder                   | Every automatic or explicit Cargo target root rejects unsafe Rust; state machine never writes after closure, stops on EOF, and checked UTF-8 replacement preserves valid prefixes.                                                                                                           | Compiler contract discovers safe roots, compiles deliberate probes against current roots, and rejects a future unannotated root. Kani covers pure pump policy; checked UTF-8 no longer contains `from_utf8_unchecked`. | No raw resource integers or pointers enter the safe crate; lifetime-bound typed native handles do enter through `AsStream`. Aggregate tracing counts retain saturating-add semantics; per-chunk checked progress is verified separately.                                                              | Historical Round 35 Rust checks covered 115/115 tests with no skips. The later current contract run found two roots, compiled six probes, and rejected all dynamic target cases.                | Safe-policy/decoder Kani passed 11 harnesses.                                                                                                                                                                           |
| `cuprum-rust` PyO3 integration                             | Generated wrappers expose stable signatures, map errors, release the GIL safely, and preserve the native resource contract.                                                                                                                                                                  | PyO3 compile UI tests and Python integration tests. Verus/Miri exclude interpreter-generated ABI code; typed Rust seams are verified separately.                                                                       | PyO3/CPython ABI, `Send`, interpreter lifecycle, and Python transport hand-off are trusted assumptions.                                                                                                                                                                                               | Round 35 Python checks recorded 1,568 passes and one skip; the extension-required suite recorded 80 passes and one skip.                                                                        | Local integration checks are green; Python lifetime establishment remains outside this evidence. Hosted Windows/macOS runs pass. The first CodeRabbit review passed with 0 findings; the second review remains pending. |
