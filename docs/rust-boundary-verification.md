...
| -------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `cuprum-native-io::adopt_writer` (`src/lib.rs:56`)                                                       | Unix `OwnedFd::from_raw_fd` (`:60`) and Windows `OwnedHandle::from_raw_handle` (`:66`) require a valid, open, uniquely transferred resource that is never concurrently closed or reused. Drop closes once on normal, error, and real-unwind paths.     | PyO3 call at `rust/cuprum-rust/src/stream_pyfunctions.rs:53`; native ownership tests start at `ownership_tests.rs:67,122` and call the constructor at `:76,135`. Kani checks typed drop counts; native tests check close and EOF. |
| `cuprum-native-io::borrow_reader` (`src/lib.rs:77`)                                                      | Unix and Windows `BorrowedStream::borrow_raw` (`:81`, `:87`) require the owner to keep the same resource live for the returned lifetime and prevent close/reuse during GIL release, cancellation, and the callback.                                    | PyO3 calls at `rust/cuprum-rust/src/stream_pyfunctions.rs:57,89`; native ownership tests at `ownership_tests.rs:67,122` exercise the typed safe `borrow` path. Safe `borrow` uses an existing `AsStream` lifetime.                |
