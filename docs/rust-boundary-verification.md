...
| Windows fixtures (`src/windows.rs:18,43`)                                                                | `CreatePipe` (`:23`) fills handle slots before `from_raw_handle` (`:28-32`) adopts them. `GetHandleInformation` (`:47`) observes state without ownership.                                                                                              | Windows tests are written; cross-Clippy passed, and hosted Windows runtime execution passed in CI runs.                                                                                                                                                          |
...
tests passed with 0 ignored. No Miri UB check is disabled, and the command uses
no `-Zmiri-*` flags. The stream tests use 16 generated cases under Miri and
disable proptest failure persistence only because Miri isolation has no usable
current directory; both settings bound runtime and do not weaken interpretation.

The Miri-compatible stream inventory covers production fallback read/write
progress and short-I/O policy, deterministic retry and error classification,
UTF-8 decoder chunk handling, byte-count accounting, and the
`with_owned_writer` normal/error drop crossing through an in-memory writer. The
following `cuprum-streams` tests are compiled out under Miri because they
require real descriptors or pipes:

- `consume_snapshot_tests`: `pure_ascii_decodes_verbatim`,
  `multibyte_sequences_split_across_buffer_boundaries`,
  `invalid_bytes_become_replacement_characters`,
  `incomplete_trailing_sequence_is_replaced_at_eof`,
  `properties::decodes_exactly_like_from_utf8_lossy`, and
  `properties::decoding_is_independent_of_the_buffer_size`.
- `io_utils/tests.rs`: `read_stream_reads_pipe_bytes`,
  `read_stream_reports_unreadable_descriptor`, `read_raw_fd_reports_eof`,
  `handle_write_returns_complete_outcome`, and
  `handle_write_reports_unwritable_descriptor`.
- `lib_tests.rs`: `consume_records_total_bytes_and_retries_on_span`,
  `pump_records_span_fields_under_error_filter`,
  `allocation_failure_inside_the_pump_span_keeps_its_context`,
  `classify_write_reports_a_completed_write`,
  `classify_write_propagates_a_fatal_error`, and
  `pump_drains_the_reader_after_the_writer_breaks`.
- `test_support_tests.rs`: `dup_as_file_preserves_the_original_owner`,
  `write_all_to_returns_an_error_for_a_pipe_read_end`, and
  `read_all_from_returns_an_error_for_a_pipe_write_end`.

The four real-descriptor splice tests,
`splice_transfers_all_bytes_between_pipes`,
`unsupported_descriptors_signal_fallback`,
`broken_pipe_drains_reader_and_reports_transferred_bytes`, and
`drain_reader_consumes_to_eof`, are excluded because Miri has no unshimmed
`libc::splice` support. PyO3/CPython remains excluded at the `cuprum-rust`
boundary; `cuprum-streams` contains no PyO3 tests.
...
latest `make boundary-miri` run passed 13 `cuprum-native-io` tests and 67
`cuprum-streams` tests, all with 0 ignored. Its two commands are kept separate
in the workflow log. PyO3/CPython, unshimmed `libc::splice`, unsupported
syscall representations, and other excluded targets remain outside that Miri
run. No UB check is disabled, and the Miri-only proptest case bound is a
runtime limit only.
