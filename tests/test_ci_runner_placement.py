...
def test_job_names_do_not_follow_their_runner(
    ...
    The hazard is event dependence, not matrix dependence. With the fork
    fallback the runner follows the event, so a name reading that condition
    renders differently on a fork's pull request and an internal one, and no
    single required context exists on both.

    A name reading a matrix key is a different thing: the values behind the key
    are literals, so the name varies per leg and is stable per event, which is
    how matrix legs get distinct contexts at all. Those jobs are listed rather
    than waved through, because a context spelling a runner label is still a
    context that a placement change would rename (shared-actions #480).

    Resolved through the matrix either way: rstest-bdd #788's contract compared
    the name's references with `runs-on`'s, which read `matrix.os` and stopped,
    while the matrix value behind that key held the fork expression.
    """
    placed = placement(workflow_name, job_name)
    shared = references(job(workflow_name, job_name).get("name")) & placed.references
    if placed.kind == "matrix":
        assert (workflow_name, job_name) in RUNNER_NAMED_JOBS or not shared, (
            f"{workflow_name}:{job_name} spells its runner in its name but is "
            "not in RUNNER_NAMED_JOBS; a placement change would rename its "
            "check context"
        )
        return
    assert not shared, (
