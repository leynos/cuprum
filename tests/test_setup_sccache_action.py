...
def _run_install(
    tmp_path: Path, backend: str, credentials: dict[str, str]
) -> StepResult:
    """Run the install step for one backend with the given runner credentials.

    The fake binary's digest is computed here and passed as the pin, which is
    what makes the step reuse it instead of downloading a release.
    ...
    return run_step(
        ...
        environment={
            ...
            "SCCACHE_SHA256": "0" * 64,
            "SCCACHE_BINARY_SHA256": hashlib.sha256(binary.read_bytes()).hexdigest(),
            "SCCACHE_LOCAL_CACHE_SIZE": CACHE_SIZE,
...
def test_the_local_backend_exports_only_the_directory(tmp_path: Path) -> None:
    ...
    result = _run_install(tmp_path, "local", {})

    assert result.returncode == 0, result.stderr
    assert result.exported.get("SCCACHE_DIR") == f"{tmp_path}/.cache/sccache", (
    ...
    assert result.exported.get("RUSTC_WRAPPER") == f"{tmp_path}/.local/bin/sccache", (
...
def test_the_actions_backend_exports_only_the_actions_switch(tmp_path: Path) -> None:
    ...
    result = _run_install(tmp_path, "gha", PROXY_CREDENTIALS)
...
def test_the_actions_backend_fails_without_either_credential(
    ...
    result = _run_install(tmp_path, "gha", credentials)
...
def test_an_unknown_backend_fails_before_anything_is_exported(tmp_path: Path) -> None:
    ...
    result = _run_install(tmp_path, "s3", PROXY_CREDENTIALS)
